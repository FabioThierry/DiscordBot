...
import SiteBScraper from '../site-C.scraper.js'
...
describe('SiteBScraper', () => {
    it('should scrape data successfully 1 ', async () => {
        ...
        const htmlPath = path.resolve(__dirname, '__mock__/rainofsnow.html')

        const htmlContent = fs.readFileSync(htmlPath, 'utf-8').trim()

        const testUrl =
            'https://rainofsnow.com/i-got-engaged-to-the-blind-duke/page/2/'
        ...
        expect(result.title).toBe('I Got Engaged To The Blind Duke')
        expect(result.img).toBe(
            'https://rainofsnow.com/wp-content/uploads/2024/04/blind-duke-2.jpeg',
        )
        expect(result.lastChapter.title).toBe('134. Chapter 134')
        expect(result.lastChapter.number).toBe(134)
        expect(result.lastChapter.url).toBe(
            'https://rainofsnow.com/chapters/blind-duke-ch134/?novelid=59726',
        )
        expect(result.lastChapter.date).toBe('March 21, 2025')
    })

    it('should scrape data successfully 2 ', async () => {
        // Lendo o arquivo HTML da pasta __mock__
        const htmlPath = path.resolve(__dirname, '__mock__/rainofsnow2.html')

        const htmlContent = fs.readFileSync(htmlPath, 'utf-8').trim()

        const testUrl = 'https://rainofsnow.com/eternal-green/'
        // Simulando a resposta do axios
        axios.get.mockResolvedValue({ data: htmlContent })
        ...
        expect(result.title).toBe('Eternal Green')
        expect(result.img).toBe(
            'https://rainofsnow.com/wp-content/uploads/2024/09/eternal-Green.webp',
        )
        expect(result.lastChapter.title).toBe('50. Chapter 49')
        expect(result.lastChapter.number).toBe(49)
        expect(result.lastChapter.url).toBe(
            'https://rainofsnow.com/chapters/eternal-green-c49/?novelid=63225',
        )
        expect(result.lastChapter.date).toBe('March 12, 2025')
    })
